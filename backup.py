    ...
    return [
        ...
        dcc.Graph(
            ...
            figure={
                'data':[
                    go.Histogram(
                        x=dfTips[(dfTips[ddlhist] >= minimum) & (dfTips[ddlhist] <= maximum)][ddlhist],
                        marker=dict(color=colorHist['normal']),
                        name='Normal'
                    ),
                    go.Histogram(
                        x=dfTips[(dfTips[ddlhist] < minimum) | (dfTips[ddlhist] > maximum)][ddlhist],
                        marker=dict(color=colorHist['notNormal']),
                        name='Not normal'
                    )
                ],
                'layout':
                    go.Layout(
                        xaxis=dict(title=ddlx),
                        yaxis=dict(title=ddlhist),
                        bargap=0.2
                    )
            }
    )]
